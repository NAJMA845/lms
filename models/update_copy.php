...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($check_result && $check_result->num_rows > 0) {
        ...
        //Check before insert whether there is another copy exist with the same number
        $check_sql = "SELECT 1 FROM book_copies WHERE copy_no = '$updated_value'";
        $check_result = $conn->query($check_sql);
        if ($check_result && $check_result->num_rows == 0) {
            ...
            $result['error']='Copy number already exists';
            echo "error";
        }
    }
}
